...
namespace
{
  void printTree(std::ostream& out, const lebedev::AVLtree< int, std::string >& tree)
  {
...
{
  lebedev::AVLtree< int, std::string > tree;
...
BOOST_AUTO_TEST_CASE(copy_constructor)
{
  ...
  lebedev::AVLtree< int, std::string > treeCopy(tree);
  treeCopy.insert({5, "5"});
...
BOOST_AUTO_TEST_CASE(move_constructor)
{
  ...
  tree.insert({3, "3"});
  tree.insert({4, "4"});
  lebedev::AVLtree< int, std::string > treeMove(std::move(tree));
...
BOOST_AUTO_TEST_CASE(copy_operator)
{
  ...
  tree.insert({2, "2"});
  tree.insert({3, "3"});
  lebedev::AVLtree< int, std::string > treeCopy;
  treeCopy = tree;
  std::ostringstream out1;
  printTree(out1, tree);
  std::ostringstream out2;
...
BOOST_AUTO_TEST_CASE(move_operator)
{
  ...
  tree.insert({2, "2"});
  tree.insert({3, "3"});
  lebedev::AVLtree< int, std::string > treeMove;
